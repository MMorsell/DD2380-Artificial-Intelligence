    ...
    def q_learning(self):
        ...
        while episode <= self.episode_max :
            ...
            while not end_episode:
                ...
                Q[s_current, indexWithHighestValue] = (1 - self.alpha) * Q[s_current, indexWithHighestValue] + self.alpha *(R + (self.gamma * np.nanmax(Q[s_next]))) 
                # print(Q[s_current])
                # print((1 - self.alpha) * Q[s_current, indexWithHighestValue] + self.alpha *(R + (self.gamma * max(Q[s_next]))))

                #R + discount * max(Q[s_next])
                # ADD YOUR CODE SNIPPET BETWEEN EX. 2.2
